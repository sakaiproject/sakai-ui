import "../sui-lti-iframe.js";
import { expect, fixture, waitUntil } from "@open-wc/testing";
import { html } from "lit";
import * as data from "./data.js";

describe("sui-lti-iframe tests", () => {
  ...
  it ("renders correctly", async () => {
    ...
    let el = await fixture(html`<sui-lti-iframe></sui-lti-iframe>`);
    await waitUntil(() => el.i18n);
    expect(el.querySelector("div.sakai-iframe-launcn")).to.not.exist;


    el = await fixture(html`<sui-lti-iframe new-window-text="Eggs" launch-url="${launchUrl}"></sui-lti-iframe>`);
    await waitUntil(() => el.i18n);
    ...
    el = await fixture(html`<sui-lti-iframe new-window-text="Eggs" launch-url="${launchUrl}" allow-resize="true"></sui-lti-iframe>`);
    await waitUntil(() => el.i18n);
  ...
  it ("is accessible", async () => {
    ...
    const el = await fixture(html`<sui-lti-iframe new-window-text="Eggs" launch-url="${launchUrl}"></sui-lti-iframe>`);
