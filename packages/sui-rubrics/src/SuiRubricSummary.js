...
import { rubricsApiMixin } from "./SuiRubricsApiMixin.js";
 ...
 */
export class SuiRubricSummary extends rubricsApiMixin(RubricsElement) {
    ...
    if (this.summaryType === "criteria") {
      return html`
        <div class="pull-right collapse-toggle-buttons">
          <button type="button" @click=${this._expandAll}>${this.i18n.expand_all}</button>
          <button type="button" @click=${this._collapseAll}>${this.i18n.collapse_all}</button>
        </div>
        <h3>${this.i18n.criteria_summary}</h3>
        ${!this.allEvaluations?.length ? html`
          <div class="sak-banner-warn">${this.i18n.no_evaluations_warning}</div>
        ` : html`
                  ...
                  <div id="collapse${c.id}" class="panel-collapse collapse">
                    <div class="card-body">
                          ...
                          <tr>
                            ${c.ratings.map(r => html`
                              <th class="rubrics-summary-table-cell">
                                  <div>${r.points} <span>${this.i18n.points}</span></div>
                              ...
                              ${this.association.parameters.fineTunePoints && this._getCustomCount(c.id, r.points) > 0 ? html`
                                <th class="rubrics-summary-table-cell"><span>${this.i18n.adjusted_score}</span></th>
                              ` : ""}
                            `)}
                            <th style="display:none" class="rubrics-summary-table-cell rubrics-summary-average-cell"><span>${this.i18n.average}</span></th>
                            <th style="display:none" class="rubrics-summary-table-cell "><span>${this.i18n.median}</span></th>
                            <th style="display:none" class="rubrics-summary-table-cell "><span>${this.i18n.stdev}</span></th>
                          </tr>
                          <tr>
                            ${c.ratings.map(r => html`
                      ...
                      <dl class="dl-horizontal">
                          <dt><span>${this.i18n.average}</span></dt>
                          <dd>${this._getPointsAverage(c.id)}</dd>
                          <dt><span>${this.i18n.median}</span></dt>
                          <dd>${this._getPointsMedian(c.id)}</dd>
                          <dt><span>${this.i18n.stdev}</span></dt>
                          <dd>${this._getPointsStdev(c.id)}</dd>
                      </dl>
                    </div>
                  </div>
                </div>
          ...
          `)}
          <div><span>${this.i18n.adjusted_score_warning}</span></div>
        `}
      `;
    }

    return html`
      <h3><span>${this.i18n.student_summary}</span></h3>
      ${!this.allEvaluations?.length ? html`
      <div class="sak-banner-warn">
          <span>${this.i18n.no_evaluations_warning}</span>
      </div>
      ` : html`
      <div class="table">
        <table id="student-summary" class="rubrics-summary-table table table-bordered table-condensed">
          <thead>
            <tr>
              <th class="rubrics-summary-table-cell rubrics-summary-table-cell-wide"><span>${this.i18n.student_name}</span></th>
              ${this.criteria.map(c => html`<th class="rubrics-summary-table-cell" >${c.title}</th>`)}
              <th class="rubrics-summary-table-cell rubrics-summary-average-cell"><span>${this.i18n.score}</span></th>
            </tr>
          </thead>
          <tbody>
          ...
          <tfoot>
            <tr>
              <th class="rubrics-summary-table-cell rubrics-summary-table-cell-wide rubrics-summary-average-row " ><span>${this.i18n.average}</span></th>
              ${this.criteria.map(c => html`
