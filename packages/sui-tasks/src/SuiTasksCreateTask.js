...
import { SuiDialogContent } from "@sakai-ui/sui-dialog-content";
import "@sakai-ui/sui-button";
import "@sakai-ui/sui-date-picker";
 ...
 */
export class SuiTasksCreateTask extends SuiDialogContent {
  ...
  save() {
    ...
    this.task.notes = this.shadowRoot.getElementById("notes")?.value;
  ...
  set task(value) {
    ...
    this.updateComplete.then(() => {
      ...
      priorityEl && (priorityEl.value = value.priority);

      const notesEditor = this.shadowRoot.getElementById("notes");
      if (notesEditor) {
        notesEditor.value = value.notes;
        notesEditor.disabled = value.system;
      }
      if (disableFields) {
        descriptionEl && (descriptionEl.disabled = true);
        datePicker.disabled = true;
      }
  ...
  content() {
    ...
    return html` 
      ...
      <div class="input">
        <textarea id="notes" .value=${this.task.notes}></textarea>
      </div>
  ...
  buttons() {
    ...
    return html`
      <sui-button @click=${this.save} primary>${this.task.taskId == "" ? this.i18n.add : this.i18n.save}</sui-button>
    `;
  }

  static get styles() {

    return [ SuiDialogContent.styles,
      css`
