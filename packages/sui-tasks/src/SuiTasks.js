...
import { SuiPageableElement } from "@sakai-ui/sui-pageable-element";
import "@sakai-ui/sui-icon";
import "@lion/dialog";
import "../sui-tasks-create-task.js";
import "@sakai-ui/sui-date-picker";

export class SuiTasks extends SuiPageableElement {
    ...
    return html`
      ...
      <div id="add-block">
        <lion-dialog id="add-edit-dialog">
          ...
          <sui-tasks-create-task class="dialog-content"
            id="create-task"
            slot="content"
            site-id="${this.siteId}"
            user-id="${this.userId}"
          ...
          </sui-tasks-create-task>

          <div slot="invoker">
            <a @click=${this.add} href="javascript:;" title="${this.i18n.add_task}" aria-label="${this.i18n.add_task}">
              <button type="button" class="add-task-button"><sui-icon type="add" size="small"></sui-icon>${this.i18n.add_task}</button>
            </a>
          </div>
        ...
        ${this.dataPage.filter(t => t.visible).map((t, i) => html`
          <div class="priority-block priority_${t.priority} cell ${i % 2 === 0 ? "even" : "odd"}">
            <div tabindex="0" title="${this.i18n[`priority_${t.priority}_tooltip`]}" aria-label="${this.i18n[`priority_${t.priority}_tooltip`]}">
              <sui-icon size="small" type="priority"></sui-icon>
            </div>
          </div>
          ...
          <div class="link-block cell ${i % 2 === 0 ? "even" : "odd"}">
            <div class="edit">
              <a href="javascript:;"
                ...
                <sui-icon type="edit" size="small"></sui-icon>
              </a>
            </div>
            ${t.softDeleted ? html`
              <div class="delete">
                <a href="javascript:;"
                  ...
                  <sui-icon type="delete" size="small"></sui-icon>
                </a>
              </div>
              <div class="restore">
                <a href="javascript:;"
                  ...
                  <sui-icon type="restore" size="small"></sui-icon>
                </a>
              </div>
            ` : html`
              <div class="delete">
